...
def woe_iv(_df, target):
    ...
    for var in var_list:
        ...
        df_woe2 = pd.DataFrame({'feature': var, 'type': df_woe['type'],
                                ...
                                'cnt': df_woe['count'], 'resp': df_woe['sum']
                                })
        df_woe2['resp_rate'] = (df_woe2.resp / df_woe2.cnt) * 100
        df_woe2['resp_pct'] = (df_woe2.resp / df_woe2.resp.sum()) * 100
        df_woe2['nonresp_pct'] = ((df_woe2.cnt - df_woe2.resp) / (
                df_woe2.cnt.sum() - df_woe2.resp.sum())) * 100
        df_woe2['woe'] = np.log(df_woe2.nonresp_pct / df_woe2.resp_pct)
        df_woe2['iv'] = ((df_woe2.nonresp_pct - df_woe2.resp_pct) *
                         df_woe2.woe / 100).sum()
